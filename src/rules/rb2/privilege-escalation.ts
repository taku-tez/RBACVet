...
export const RB2004: Rule = {
  ...
  check(ctx: RuleContext): Violation[] {
    ...
    for (const role of allRoles(ctx)) {
      for (const rule of role.rules) {
        const targetsRbac = rbacResources.some(r => hasResource(rule, r));
...
export const RB2005: Rule = {
  ...
  check(ctx: RuleContext): Violation[] {
    ...
    for (const role of allRoles(ctx)) {
      for (const rule of role.rules) {
        const targetsBinding = bindingResources.some(r => hasResource(rule, r));
        const hasWrite = WRITE_VERBS.some(v => hasVerb(rule, v));
...
export const RB2_RULES: Rule[] = [
  ...
  RB2006, RB2007, RB2008, RB2009, RB2010,
];
