...
const privilege_escalation_1 = require("../rules/rb2/privilege-escalation");
...
function computeScores(graph, violations, threshold, trusted) {
    const scores = [];
    const trustedSet = new Set(trusted);
    const violationsBySA = new Map();
    ...
    for (const sa of graph.serviceAccounts.values()) {
        ...
        const boundBindings = allBindings.filter(b => b.subjects.some(s => s.kind === 'ServiceAccount' && s.name === sa.metadata.name &&
        ...
        for (const binding of boundBindings) {
            ...
            // secrets read
            const hasSecretsRead = role.rules.some(r => (r.resources.includes('secrets') || r.resources.includes('*')) &&
            ...
            // pods/exec
            const hasExec = role.rules.some(r => r.resources.includes('pods/exec') || r.resources.includes('*'));
            if (hasExec) {
                score += 20;
        ...
        const chain = (0, privilege_escalation_1.findEscalationChain)(sa.metadata.name, ns, graph, trustedSet);
        if (chain) {
